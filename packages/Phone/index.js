...
mp.events.add("server:phone:getSms", (player) => {
  if(mp.players.exists(player)) {
    mp.events.call('server:mysql:task', "SELECT * FROM phone_sms WHERE targetnumber = ? AND multi = '0' ORDER BY id DESC", [player.data.phoneNumber], () => {
      if (resUp.length > 0) {
        var c = 0;
        let names = [];
        resUp.forEach(function(contacts) {
...
mp.events.add("server:phone:getSmsContact", (player,number) => {
  if(mp.players.exists(player)) {
    mp.events.call('server:mysql:task', "SELECT * FROM phone_sms WHERE targetnumber = ? AND phonenumber = ?",[player.data.phoneNumber,number], () => {
      mp.events.call('server:mysql:task', "SELECT * FROM phone_sms WHERE pnid = ? ORDER BY id DESC", [res[0].pnid], () => {
        if (resUp.length > 0) {
          ...
          gm.mysql.handle.query("SELECT * FROM phone_sms WHERE pnid = ?",[resUp[0].pnid],function(errUp1,resUp1) {
            ...
            mp.events.call('server:mysql:task', "UPDATE phone_sms SET gelesen = '1' WHERE pnid = ? AND targetId = ?",[resUp[0].pnid,player.data.charId], () => {
...
mp.events.add("server:phone:saveContact", (player, name, number, old_name, old_number) => {
  if(mp.players.exists(player)) {
    mp.events.call('server:mysql:task', 'UPDATE phone_contacts SET contactName = ?, phoneNumber = ? WHERE contactName = ? AND phoneNumber = ? AND playerCharID = ?', [name, number, old_name, old_number, player.getVariable("id")], () => {
      if (errUp) {
        ...
        if(mp.players.exists(player)) {
          mp.events.call('server:mysql:task', "SELECT * FROM phone_contacts WHERE playerCharID = ? ORDER BY contactName asc", [player.data.charId], () => {
            let contactList = [];
            res99.forEach(function(contact) {
                let obj = {"contactName": contact.contactName};
                contactList.push(obj);
...
mp.events.add("server:Global:GetBankMoney",(player) => {
  if(mp.players.exists(player)) {
    mp.events.call('server:mysql:task', "SELECT * FROM players WHERE id = " + mysql.escape(player.getVariable("id")) +" AND faction = 'Zivilist'", () => {
      var ownerId = res[0].ownerId;
      var scode = "(SAHDJSAHD4)";
